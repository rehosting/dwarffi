...
def base_types_isf():
    return DFFI({
        ...
        "user_types": {"dummy_struct": {"kind": "struct", "size": 4, "fields": {}}},
        "enums": {"status": {"size": 4, "base": "int", "constants": {"OK": 0, "ERROR": 1}}},
        "symbols": {},
    })
...
def test_pointer_dereference_syntax(base_types_isf):
    ...
    int_buf = bytearray(4)
    int_inst = base_types_isf.from_buffer("int", int_buf)
    ...
    enum_inst = base_types_isf.from_buffer("enum status", enum_buf)
...
def test_struct_dereference_error(base_types_isf):
    ...
    struct_inst = base_types_isf.from_buffer("struct dummy_struct", struct_buf)
...
def test_float_conversion(base_types_isf):
    ...
    float_inst = base_types_isf.from_buffer("float", float_buf)
