...
def ffi_arm32_le(tmp_path):
    ...
    path = make_arch_isf(tmp_path, "arm32_le.json", {
...
def ffi_mips32_be(tmp_path):
    ...
    path = make_arch_isf(tmp_path, "mips32_be.json", {
...
def ffi_ppc64_be(tmp_path):
    ...
    path = make_arch_isf(tmp_path, "ppc64_be.json", {
        "int": {"kind": "int", "size": 4, "signed": True, "endian": "big"},
...
def test_32bit_little_endian_memory_layout(ffi_arm32_le: DFFI):
    ...
    assert struct.unpack("<i", raw_bytes[4:8])[0] == struct.unpack("<i", struct.pack("<I", 0xDEADBEEF))[0]
