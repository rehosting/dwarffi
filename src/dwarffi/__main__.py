...
if __name__ == "__main__":
    ...
    cli_parser.add_argument(
        "json_file_path", type=str, help="Path to the ISF JSON or JSON.XZ file."
    )
    cli_parser.add_argument("-v", "--verbose", action="store_true", help="Print detailed info.")
    cli_parser.add_argument(
        "--find-symbol-at",
        type=lambda x: int(x, 0),
        metavar="ADDRESS",
        help="Find symbols at address.",
    )
    cli_parser.add_argument("--test-write", action="store_true", help="Run field write test.")
    cli_parser.add_argument("--test-to-bytes", action="store_true", help="Run to_bytes() test.")
    cli_parser.add_argument(
        "--test-base-enum-instance",
        action="store_true",
    ...
    print(f"Attempting to load ISF file: {args.json_file_path} (using {_JSON_LIB_USED})")
    ...
    try:
        ...
        if args.find_symbol_at is not None:
            ...
            symbols_at_addr = isf_data.get_symbols_by_address(args.find_symbol_at)
            if symbols_at_addr:
                print(f"  Found {len(symbols_at_addr)} symbol(s) at {args.find_symbol_at:#x}:")
                for sym_obj in symbols_at_addr:
                    print(f"    - {sym_obj}")
            else:
                print(f"  No symbols found at address {args.find_symbol_at:#x}.")
        ...
        if args.verbose:
            ...
            print(f"  Number of raw base types defined: {len(isf_data._raw_base_types)}")
            print(f"  Number of raw user types defined: {len(isf_data._raw_user_types)}")
            print(f"  Number of raw enums defined: {len(isf_data._raw_enums)}")
            print(f"  Number of raw symbols defined: {len(isf_data._raw_symbols)}")
        ...
        if args.test_write or args.test_to_bytes or args.test_array_write:
            ...
            if struct_def and struct_def.size is not None:
                ...
                instance = isf_data.create_instance(struct_to_test, buffer_data)
                ...
                if args.test_array_write and "args" in struct_def.fields:
                    ...
                    print(
                        f"    Initial args_array_view[0]: {args_array_view[0] if len(args_array_view) > 0 else 'N/A'}"
                    )
